...
module W : sig
  ...
  let pp_state = function
    | Lwt.Return _ -> Printf.printf "Return"
    | Sleep _ -> Printf.printf "Sleep"
    | Fail exn -> Printf.printf "EXN:%s" (Printexc.to_string exn)

  let init rpc a = 
    let open Lwt.Syntax in 
    let t = Wraw.init rpc a in
    Printf.printf "Hello World";
    let r = t |> Rpc_lwt.T.get in
    pp_state (Lwt.state r);
    let* r = r in
    Lwt.return r
